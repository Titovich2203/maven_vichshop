...
public class LoadView
{
	...
	{
		if(type == 1){
			System.out.println(LoadView.class.getResource("/vichshop/view/"+name));
			root= fl.load(LoadView.class.getResource("/vichshop/view/"+name)); //load a simple view
			scene=new Scene(root);
		}
		else if(type == 2) // load borderpane
		{

			rootBorder= fl.load(LoadView.class.getResource("/vichshop/view/" +name));
	...
	private static void Load(int type)
	{
		try
		{
			...
			else if(type == 2) // load borderpane
			{
				...
				root= fl.load(LoadView.class.getResource("/vichshop/view/" +name));
				if(rootBorder != null){
...
/*
	public static Object getController(String nom) throws IOException {
	    if(!name.equals(""))
        {
            ...
            AnchorPane userPane = (AnchorPane) fl.load(LoadView.class.getResource("/vichshop/view/"+nom).openStream());
